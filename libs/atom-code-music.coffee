...
class AtomCodeMusic
  constructor: ->
    ...
    @keysNotes = JSON.parse(fs.readFileSync(path.join(__dirname, "keys-notes.json")))
    @subscriptions = new CompositeDisposable()
    @sheet = []
    @sheets= {}
    sheetsArray = fs.readdirSync(path.join(__dirname, "sheets"))
    for sheetName in sheetsArray
        @sheets[sheetName] = JSON.parse(fs.readFileSync(path.join(__dirname, "sheets", sheetName)))
    ...
    for timbre of timbres then do (timbre) =>
      ...
      for pitch of timbres[timbre] then do (pitch) =>
        @context.decodeAudioData(@base64Binary.decodeArrayBuffer(timbres[timbre][pitch].split("data:audio/ogg;base64,")[1]), (audioBuffer) =>
  ...
  activate: ->
    @subscriptions.add(atom.commands.add("atom-workspace", "atom-code-music:toggle": @toggle))
    @subscriptions.add(atom.commands.add("atom-text-editor", "atom-code-music:convert": @convert))
    @changeMode()
    @changeTimbre()
    @changeSheets()
    ...
    @subscriptions.add(atom.config.onDidChange("atom-code-music.customSheets", @changeSheets))
  ...
  changeMode: =>
    ...
    if @workMode is "Music Box Mode" and not @sheet?.length
      @sheet = Object.keys(@sheets)[Math.floor(Math.random() * Object.keys(@sheets).length)]

  changeTimbre: =>
    @timbre = atom.config.get("atom-code-music.timbre")
    if @timbre is "Random"
      @timbre = Object.keys(@timbresAudio)[Math.floor(Math.random() * Object.keys(@timbresAudio).length)]
    ...
    if @customSheets.length
      ...
      for customSheetName in customSheetsArray
              if customSheetName not of @sheets
                      try
                        @sheets[customSheetName] = JSON.parse(fs.readFileSync(path.join(customSheetName)))
                      catch error
                        ;
